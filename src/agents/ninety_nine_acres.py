...
log = get_logger("99acres")


class UnitMix(BaseModel):
    configuration: str = Field(description="E.g., 2 BHK, 3 BHK, Shop, Office")
    carpet_area: str = Field(description="Carpet area or super built-up area")
    price: str = Field(description="Price or price range for this configuration")


class NinetyNineAcresData(BaseModel):
...
class NinetyNineAcresAgent(BaseAgent):
    """Agent that uses DuckDuckGo to find the 99acres project page and curl_cffi+Gemini to extract details."""

    async def run(self, context_data: Dict[str, Any] = None) -> Dict[str, Any]:
        log.info("Starting NinetyNineAcresAgent...")
        project_name = (context_data or {}).get("project_name", "Poonam Estate Cluster 2")

        # ── Step 1: DuckDuckGo search ──────────────────────────────────
        log.info(f"Searching DuckDuckGo for: site:99acres.com {project_name}")
        search_url = "https://html.duckduckgo.com/html/"
        res = curl_requests.post(search_url, data={"q": f"site:99acres.com {project_name}"}, impersonate="chrome120")
        ...
        if res.status_code == 200:
            ...
            for a in soup.select("a.result__url"):
                href = a.get("href", "")
                text = a.text.strip()
                if "99acres.com/" in href or "99acres.com/" in text:
                    real_url = f"https://{text}" if "99acres.com" in text else href
                    if not real_url.startswith("http"):
                        real_url = "https://" + real_url
                    if "-project-" in real_url or "-residency-" in real_url or "estate" in real_url.lower():
        ...
        if not target_project:
            ...
            return {
                "source": SourceName.NINETY_NINE_ACRES.value,
                "data": {"error": f"Could not find 99acres page for {project_name}"},
            }

        log.info(f"Selecting 99acres URL: {target_project['href']}")
        extracted_data = {}
        ...
        try:
            ...
            for img in soup.find_all("img"):
                ...
                if src and ("jpg" in src.lower() or "png" in src.lower()) and "icon" not in src.lower():
                    if src.startswith("//"): src = "https:" + src
                    elif src.startswith("/"): src = "https://www.99acres.com" + src
                    img_urls.append(src)
            # Take a sample of images so Groq doesn't get overwhelmed
            image_context = "\n".join(list(set(img_urls))[:20])

            # ── Step 3: Groq extraction with retry ───────────────────
            log.info("Extracting structured data from 99acres with Groq...")
            prompt = (
                "Extract the following information about the real estate project "
                "from the provided 99acres page text:\n"
            ...
            response = await groq_generate_with_retry(
                contents=prompt,
                response_schema=NinetyNineAcresData,
            )
            ...
            log.error(f"99acres extraction failed: {e}")
